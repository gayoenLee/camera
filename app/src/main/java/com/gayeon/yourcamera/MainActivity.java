...
public class MainActivity extends AppCompatActivity {
   ...
   // ImageButton changeCameradirectionBtn;
    String mCameraId;
    String TAG = "메인액티비티";
     //sparseArray : integer값을 오브젝트에 매핑해주는 것.
    //특이한 점 : 각 인덱스 사이에 공간을 만들 수있다는 점(메모리를 효율적으로 사용해 성능이 좋음.hashmap방식보다.)
    //많은 아이템들을 넣고 관리하기에는 부적합.
...
/*
        ...
        //카메라 방향 전환 버튼.
        ImageButton changeCameradirectionBtn = findViewById(R.id.change_direction_btn);
        changeCameradirectionBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                 mCameraId = ""+ CameraCharacteristics.LENS_FACING_BACK;
                 Log.i(TAG, "카메라 방향 전환 버튼 클릭");
                Log.i(TAG, "카메라 아이디값 확인"+ mCameraId);
   ...
   public void init(){
    ...
    public void initCameraAndPreview() {
        ...
        try {
            //후면 카메라를 사용.
            //characteristics.LENS_FACING은 카메라의 렌즈 방향을 얻을 때 사용하는 키
            //전면 : _FRONT, value는 0
            //후면 : _BACK, value는 1
            //카메라와 관련된 모든 작업은 카메라매니저로부터 시작됨. getSystemService()함수로 얻으면 됨.
            CameraManager mCameraManager = (CameraManager) this.getSystemService(Context.CAMERA_SERVICE);
            //새로 추가한 것.
            assert mCameraManager != null;
            //CameraCharacteristics각 카메라의 정보를 담고 있는 객체.
            //카메라아이디를 카메라식별자로 해서 getCameraCharacteristics(cameraID)함수를 호출하면 위 객체를 얻을 수 있음.
            CameraCharacteristics characteristics = mCameraManager.getCameraCharacteristics(mCameraId);
            Log.i(TAG, "카메라 아이디값 확인"+ mCameraId);
            //위의 객체에 담긴 각종 정보를 추출하려면 각 정보에 해당하는 키 값을get()함수의 매개 변수로 지정하여 얻음.
            //아래 코드로 StreamConfigurationMap객체를 얻을 수 있음.여기에서 카메라에서 지원하는 크기 목록 얻을 수 있음.
            StreamConfigurationMap map = characteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
            //getOutputSizes함수에 이미지 포맷을 매개변수로 지정해 지원하는 크기 목록 이 사이즈 객체의 배열로 반환됨.
            //이 값을 이용해 사진 촬영시 사진 크기 지정 가능.
    ...
    public void takePicture() {
        try {
            ...
            Log.d("테이크픽쳐 메소드", mDeviceRotation+"");
